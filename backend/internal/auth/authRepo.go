...
type AuthRepo interface {
	...
	NewToken(ctx context.Context, userId, householdID int64, username, phone string) (string, error)
	KeyFunc() jwt.Keyfunc
...
func (a *authRepo) KeyFunc() jwt.Keyfunc {
	return func(token *jwt.Token) (interface{}, error) {
		return a.signingKey, nil
	}
}

func (a *authRepo) NewToken(ctx context.Context, userId, householdID int64, username, phone string) (string, error) {
	...
	claims := JwtClaims{
		...
		RegisteredClaims: jwt.RegisteredClaims{
			ID:        strconv.FormatInt(utils.GetSFID(), 10),
